 ...
 *  Project: MbAdmin
 * 
 *  File: /MbAdmin/Database/Data/Contacts/ContactsCheckRowValues.php
 * 
...
use MbAdmin\Database\Data\Contacts\ContactsCheckNameUnique;

class ContactsCheckRowValues
{
    ...
    public function checkRowValues( $dataController, $id, $values, $isUpdate ){
        ...
        if( $isUpdate ){
            ...
            $currentRow = $dataController->getRowById( 'contacts', $id );
            if( isset( $currentRow['criticalError'] ) || !$currentRow  ){
                $this->debugger->LogInformation( 'Error ContactsCheckRowValues no current row. For id: ' . $id );
            ...
            if( $currentRow['data']['updated'] != $values['updated'] ){
                $this->debugger->LogInformation( 'ContactsCheckRowValues update, dataOutOfDate ' .  PHP_EOL .
                                                 'rowUpdated: ' .$currentRow['data']['updated'] . PHP_EOL .
        ...
        $checkNameUniqueController = new ContactsCheckNameUnique( $this->debugger );
        // done
