 ...
 *  Project: MbAdmin
 * 
 *  File: /MbAdmin/Database/Data/Tasks/TasksCheckStartTimeOverlap.php
 * 
 *  Last Revision:  16-01-2017
 * 
 *  Purpose: 
 *          this class checks if the start time of a task overlaps with
...
class TasksCheckStartTimeOverlap
{
    ...
    public function checkStartTimeOverlap( $dataController, $id, $values, $isUpdate ){
        $result = [];
        ...
        if( $row && isset( $row[0] ) ){
            // debug info
            $this->debugger->LogInformation( 'TasksCheckStartTimeOverlap count: ' . $row[0]['count'] );
            // found records
            if( $row[0]['count'] > 0 ){
                // add error to result
